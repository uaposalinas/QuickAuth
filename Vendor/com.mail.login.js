...
function InitMailLogService(){
    ...
    class MailInfo{
        ...
        constructor(Name, UserName, Mail, Token){
    ...
    const NewLoginRequest = new MailInfo(AccountName, GetUserName, GetUserMail, CreateToken(60));
    ...
    fetch('../Controllers/com.sendmail.login.php', {
        ...
        body: `AccountName=${encodeURIComponent(NewLoginRequest.Name)}&UserName=${encodeURIComponent(NewLoginRequest.UserName)}&Mail=${encodeURIComponent(NewLoginRequest.Mail)}&Token=${NewLoginRequest.Token}`,
...
function CreateToken(l){
    ...
    for(let Aument = 0; Aument < TotalMatched; Aument++){
        ...
        const Replace = Token.replace("0", Letter[ Math.floor( Math.random() * 27 ) ]);
        Token = Replace;
